...
DEFAULT_CONNECTION_TIMEOUT = 10  # in seconds
...
class AntiVirus(ServiceBase):
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in sets.not_done:
                    ...
                    if host.mercy_counter > self.mercy_limit:
                        self.log.warning(
                            f"{host.group} host {host.ip}:{host.port}'s mercy counter exceeded the limit. Going to sleep for {self.sleep_time}s.")
                        Thread(target=host.sleep, args=[self.sleep_time], daemon=True).start()

                # Reset the mercy counter on a run that completed in time. This can be either a successful run or a
                # run where an error was raised and the host was put to sleep.
                not_completed_futures = [future for future in sets.not_done]
                completed_futures = [future for future in futures if future not in not_completed_futures]
                for completed_future in completed_futures:
                    host = futures[completed_future]
    ...
    def _scan_file(self, host: AntiVirusHost, file_hash: str,
        ...
        try:
            ...
            # If a version was attempted to be retrieved, but a connection was unsuccessful, do not try to scan file.
            if host.method == ICAP_METHOD and not host.host_client.scan_details.no_version and not host.host_client.client.successful_connection:
                pass
            else:
